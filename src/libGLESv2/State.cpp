...
void State::getIntegerv(GLint clientVersion, GLenum pname, GLint *params)
{
      ...
      case GL_SAMPLES:
        {
            ...
            if (framebuffer->completeness(clientVersion) == GL_FRAMEBUFFER_COMPLETE)
            {
                switch (pname)
                {
                  case GL_SAMPLE_BUFFERS:
                    if (framebuffer->getSamples(clientVersion) != 0)
                    {
                        *params = 1;
                    }
                    else
                    ...
                    break;
                  case GL_SAMPLES:
                    *params = framebuffer->getSamples(clientVersion);
                    break;
                }
            }
            else
