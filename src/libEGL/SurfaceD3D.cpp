...
//static
SurfaceD3D *SurfaceD3D::makeSurfaceD3D(Surface *surface)
{
    return makeSurfaceD3D(surface->getImplementation());
}

SurfaceD3D *SurfaceD3D::createOffscreen(Display *display, const Config *config, EGLClientBuffer shareHandle,
...
SurfaceD3D::SurfaceD3D(Display *display, const Config *config, EGLint width, EGLint height,
...
void SurfaceD3D::release()
{
    delete mSwapChain;
    mSwapChain = NULL;

    // function parameter is ignored
    releaseTexImage(0);
}

//TODO(jmadill): this method should be in the GL layer
Error SurfaceD3D::releaseTexImage(EGLint)
{
    if (mTexture)
    {
...
Error SurfaceD3D::resetSwapChain()
{
    ASSERT(!mSwapChain);
...
Error SurfaceD3D::resizeSwapChain(int backbufferWidth, int backbufferHeight)
{
...
Error SurfaceD3D::resetSwapChain(int backbufferWidth, int backbufferHeight)
{
    ASSERT(backbufferWidth >= 0 && backbufferHeight >= 0);
    ASSERT(mSwapChain);
...
Error SurfaceD3D::swapRect(EGLint x, EGLint y, EGLint width, EGLint height)
{
    if (!mSwapChain)
    {
        return Error(EGL_SUCCESS);
...
Error SurfaceD3D::swap()
{
    return swapRect(0, 0, mWidth, mHeight);
}

Error SurfaceD3D::postSubBuffer(EGLint x, EGLint y, EGLint width, EGLint height)
{
...
egl::Error SurfaceD3D::bindTexImage(EGLint buffer)
{
    return Error(EGL_SUCCESS);
}

Error SurfaceD3D::querySurfacePointerANGLE(EGLint attribute, void **value)
{
