...
void ResourceVk::onResourceRead(vk::CommandBufferNode *renderNode, Serial serial)
{
    if (isCurrentlyRecording(serial))
    {
        renderNode->addDependency(getCurrentWriteNode(serial));
    }
    else
    {
        updateQueueSerial(serial);
    ...
    mCurrentReadNodes.push_back(renderNode);
}
