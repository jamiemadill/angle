    ...
    GLsizei numIndicesPerInstance)
{
         ...
         ++attribIndex)
    {
        ...
        if (attribIndex < mCurrentAttributes.size() && mCurrentAttributes[attribIndex]->active)
        {
            ...
            {
                ASSERT(attrib.vertexBuffer);
                buffer = GetAs<VertexBuffer11>(attrib.vertexBuffer)->getBuffer();
            }
                                              ...
                                              ID3D11InputLayout **inputLayoutOut)
{
    ...
    if (programUsesInstancedPointSprites)
    {
        ...
        {
            if (mCurrentAttributes[elementIndex]->active)
            {
