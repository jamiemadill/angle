                                                      ...
                                                      GLsizei instances)
{
    ...
    // TODO(jmadill): can we clean this up?
    for (size_t attribIndex = 0; attribIndex < attribs.size(); ++attribIndex)
    {
        mTranslatedAttribs[attribIndex].active = program->isAttribLocationActive(attribIndex);
    }
    ...
    if (!mDirtyAttribIndexes.empty())
    {
        ...
        for (size_t dirtyAttribIndex : dirtyAttribs)
        {
            ...
            translatedAttrib->attribute        = &mData.getVertexAttribute(dirtyAttribIndex);
    ...
    if (!mDynamicAttribIndexes.empty())
    {
        for (size_t dynamicAttribIndex : mDynamicAttribIndexes)
        {
            ...
            // Record basic attrib info
            dynamicAttrib->attribute        = &mData.getVertexAttribute(dynamicAttribIndex);
