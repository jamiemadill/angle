...
class AttributeSorter final
{
  public:
    AttributeSorter(const AttribIndexArray &originalIndexes) : mOriginalIndexes(&originalIndexes) {}

    bool operator()(int a, int b)
    {
        int indexA = (*mOriginalIndexes)[a];
        int indexB = (*mOriginalIndexes)[b];
  ...
  private:
    const AttribIndexArray *mOriginalIndexes;
};
...
LinkResult ProgramD3D::load(gl::InfoLog &infoLog, gl::BinaryInputStream *stream)
{
    ...
    for (int i = 0; i < mAttribLocationToD3DSemantic.size(); ++i)
    {
        stream->readInt(&mAttribLocationToD3DSemantic[i]);
    }
