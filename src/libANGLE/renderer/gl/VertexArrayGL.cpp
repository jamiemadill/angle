...
size_t AttribBitIndex(uint64_t bit, uint64_t offsetBit)
{
    uint64_t baseBit = bit >> angle::BitToIndex(offsetBit);
    return angle::BitToIndex(baseBit);
}

} // anonymous namespace

VertexArrayGL::VertexArrayGL(const VertexArray::Data &data, const FunctionsGL *functions, StateManagerGL *stateManager)
...
gl::Error VertexArrayGL::syncDrawState(const std::vector<GLuint> &activeAttribLocations, GLint first, GLsizei count, GLenum type, const GLvoid *indices, const GLvoid **outIndices) const
{
    ...
    if (type != GL_NONE)
    {
        Error error = syncIndexData(count, type, indices, attributesNeedStreaming, &indexRange, outIndices);
        if (error.isError())
        {
            return error;
        }
...
Error VertexArrayGL::syncIndexData(GLsizei count, GLenum type, const GLvoid *indices, bool attributesNeedStreaming,
                                       RangeUI *outIndexRange, const GLvoid **outIndices) const
{
    ...
    if (elementArrayBuffer != nullptr)
    {
        ...
        if (attributesNeedStreaming)
        {
            ...
            Error error = mData.getElementArrayBuffer()->getIndexRange(type, static_cast<size_t>(elementArrayBufferOffset), count, outIndexRange);
            if (error.isError())
            {
                return error;
            }
...
void VertexArrayGL::updateAttribPointer(size_t attribIndex)
{
    ...
    if (attrib.pureInteger)
    {
        mFunctions->vertexAttribIPointer(
            attribIndex, attrib.size, attrib.type, attrib.stride, attrib.pointer);
    }
    else
    {
        mFunctions->vertexAttribPointer(
            attribIndex, attrib.size, attrib.type, attrib.normalized, attrib.stride, attrib.pointer);
    }

    mAppliedAttributes[attribIndex] = attrib;
}

void VertexArrayGL::syncState(StateChangeBits *dirtyBits)
{
    for (uint64_t dirtyBit : *dirtyBits)
    {
        if (dirtyBit == VertexArray::DIRTY_BIT_ELEMENT_ARRAY_BUFFER)
        {
            // TODO?
                 ...
                 dirtyBit <= VertexArray::DIRTY_BIT_ATTRIB_15_ENABLED)
        {
            size_t attribIndex = AttribBitIndex(dirtyBit, VertexArray::DIRTY_BIT_ATTRIB_0_ENABLED);
        ...
        {
            size_t attribIndex = AttribBitIndex(dirtyBit, VertexArray::DIRTY_BIT_ATTRIB_0_POINTER);
        ...
        {
            size_t attribIndex = AttribBitIndex(dirtyBit, VertexArray::DIRTY_BIT_ATTRIB_0_DIVISOR);
            const VertexAttribute &attrib = mData.getVertexAttribute(attribIndex);
